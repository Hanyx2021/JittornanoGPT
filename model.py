...
class LayerNorm(nn.Module):
    ...
    def __init__(self, ndim, bias):
        super().__init__()
        self.weight = nn.Parameter(jt.ones(ndim))
        self.bias = nn.Parameter(jt.zeros(ndim)) if bias else 0

    def execute(self, input):
        return jt.nn.layer_norm(input, self.weight.shape, self.weight, self.bias, 1e-5)
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("bias", jt.tril(jt.ones(config.block_size, config.block_size))
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer[0].weight = self.lm_head.weight # https://paperswithcode.com/method/weight-tying
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('c_proj.weight'):
                jt.nn.init.gauss_(p, mean=0.0, std=0.02/math.sqrt(2 * config.n_layer))
    ...
    def get_num_params(self, non_embedding=True):
        ...
        if non_embedding:
            n_params -= self.transformer[1].weight.numel()
        return n_params

    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            jt.nn.init.gauss_(module.weight, mean=0.0, std=0.02)
            if module.bias is not None:
                jt.nn.init.zero_(module.bias)
        elif isinstance(module, nn.Embedding):
            jt.nn.init.gauss_(module.weight, mean=0.0, std=0.02)

    def execute(self, idx, targets=None):
        # device = idx.device
        b, t = idx.size()
        ...
        for block in self.transformer[3]:
            x = block(x)
        x = self.transformer[4](x)

        if targets is not None:
            # if we are given some desired targets also calculate the loss
            logits = self.lm_head(x)
            loss = nn.cross_entropy_loss(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
        else:
    ...
    def crop_block_size(self, block_size):
        ...
        self.transformer[1].weight = nn.Parameter(self.transformer[1].weight[:block_size])
        for block in self.transformer[3]:
